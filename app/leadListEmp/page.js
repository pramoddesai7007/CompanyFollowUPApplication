...
import { faPenToSquare, faTrash, faEye, faXmark, faFileExcel } from '@fortawesome/free-solid-svg-icons';
...
const LeadListEmp = () => {
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      formData.append('customerName', editedLead.customerName);
      formData.append('companyName', editedLead.companyName);
      formData.append('contactNo', editedLead.contactNo);
      formData.append('email', editedLead.email);
      formData.append('description', editedLead.description);
      formData.append('ownerName', editedLead.ownerName);
      formData.append('website', editedLead.website);

      // Append the image file if it exists
      if (editedLead.leadPictureFile) {
        formData.append('leadPicture', editedLead.leadPictureFile);
      }

      // Append a flag to remove the image if necessary
      if (!editedLead.leadPicture && !editedLead.leadPictureFile) {
        formData.append('removeLeadPicture', 'true');
      }
      ...
      const response = await axios.put(`http://103.159.85.246:4000/api/lead/editLead/${editedLead._id}`, formData, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        setLeads((prevLeads) =>
          prevLeads.map((lead) => (lead._id === editedLead._id ? response.data : lead))
        );
      } else {
  ...
  return (
    <>
      ...
      <div className="m-5 pl-5 md:pl-72 mt-20">
              ...
              {leads.length > 0 ? (
                currentLeads.map((lead, index) => (
                  <tr key={lead._id} className='text-sm'>
                    <td className="px-4 py-1 border text-center border-violet-300">{calculateSerialNumber(index)}</td>
                    <td className="px-4 py-1 border text-center border-violet-300">{lead.customerName}</td>
                    <td className="px-4 py-1 border text-center border-violet-300">{lead.description}</td>
                    <td className="px-4 py-1 border text-center border-violet-300">{lead.contactNo}</td>
                    <td className="px-3 py-1 border text-center border-violet-300">{lead.email}</td>
                    <td className="px-4 py-1 border text-center border-violet-300">
        ...
        {isEditModalOpen && (
          <div className="fixed inset-0 flex items-center justify-center z-50" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
            <div className="modal-container bg-white max-w-5xl w-1/2 mx-auto p-5 sm:p-6 rounded shadow-lg">
              <div className="p-4 text-center">
                ...
                {editedLead && (
                  <div className='text-sm md:text-sm'>
                    <div className='flex space-x-6 w-full'>
                      <div className="mb-2">
                        ...
                        <input
                          ...
                          className="border border-gray-200 p-1 lg:w-72 rounded"
                        />
                      </div>
                      <div className="mb-2">
                    ...
                    <div className="mb-2">
                      <label htmlFor="leadPicture" className="text-left justify-center block mb-2">
                        Lead Picture
                      </label>
                      <div className='relative'>
                      {editedLead.leadPicture && (

                        <div className=" mb-2">
                          <div className='relative'>
                            <img src={`http://103.159.85.246:4000/${editedLead.leadPicture}`} alt="Lead" className="w-48 h-auto relative " />
                          </div>
                          <div
                               className=" "
                             >  
                               <FontAwesomeIcon icon={faXmark} className='w-4 h-4  items-center text-center  absolute top-3 left-40 bg-red-600 cursor-pointer text-white p-1 rounded-full' onClick={() => setEditedLead({ ...editedLead, leadPicture: '' })}/>
                             </div>
                        </div>
                      )}
                      </div>
                      <input
                        type="file"
                        name="leadPicture"
                        onChange={(e) => setEditedLead({ ...editedLead, leadPictureFile: e.target.files[0] })}
