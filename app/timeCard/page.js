...
const TimeCard = () => {
  ...
  useEffect(() => {
    // Fetch salaries for today's date on component mount
    handleFetchSalaries();
  }, []);
            ...
            <table className="min-w-full table-auto">
              <thead className='bg-orange-500 text-white text-sm' >
                <tr>
                  ...
                  <th className="px-4 py-1.5 text-center">EmployeeName</th>
                ...
                {getSalariesForCurrentPage().map((salary) =>
                  salary.clockRecords.map((record) => (
                    <tr key={serialNumber} className='cursor-pointer text-sm hover:bg-gray-300'>
                      ...
                      <td className="px-4 py-1 text-center">{salary.name}</td>
