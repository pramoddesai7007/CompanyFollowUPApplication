...
// import NavSideEmp from '@/app/components/NavSideEmp';


// const EditFormEmp = ({ params }) => {
...
//             const response = await fetch(`http://103.159.85.246:4000/api/task/editEmp/${taskId}`, {
//                 method: 'PUT',
//                 headers: {
...
const EditForm = ({ params }) => {
    ...
    const [isErrorModalOpen, setIsErrorModalOpen] = useState(false); // Add state for error modal
    const [error, setError] = useState(''); // Add state for error message
    const [pictureFiles, setPictureFiles] = useState([]); // State for the new picture files
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (pictureFiles.length > 0) {
            pictureFiles.forEach((file, index) => {
                formData.append(`pictures`, file); // Append each picture file with the same key
            });
        } else {
            // Append existing pictures if no new files are added
            taskData.pictures.forEach((picture, index) => {
    ...
    const handlePictureChange = (e) => {
        ...
        setPictureFiles((prevFiles) => [...prevFiles, ...newFiles]);
    };


    const handleRemovePicture = (index) => {
        const updatedPictures = [...taskData.pictures];
        updatedPictures.splice(index, 1);
        setTaskData({ ...taskData, pictures: updatedPictures });
    };
    ...
    const handleRemoveNewPicture = (index) => {
        setPictureFiles((prevFiles) => prevFiles.filter((_, i) => i !== index));
    };
                ...
                <div className="fixed inset-0 flex items-center justify-center z-50" style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
                    <div className="modal-container bg-white sm:w-96 sm:p-6 rounded shadow-lg">
                        <div className="p-2 text-center">
                            ...
                            <h3 className="mb-5 text-lg font-semibold text-gray-800">Task Updated Successfully</h3>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="w-full md:flex justify-center items-center min-h-screen md:mt-5 md:pl-28 bg-slate-50">
                <div className="w-full md:max-w-2xl overflow-x-auto border border-gray-200 rounded-lg p-5 bg-white mt-16">
                    ...
                    <form onSubmit={handleFormSubmit} className="grid grid-cols-2 gap-4">
                        ...
                        <div className="mb-4">
                            <label htmlFor="pictures" className="block font-semibold text-xs lg:text-sm">Pictures</label>
                            <div className="border-2 border-dashed border-gray-200 rounded-md p-3 w-full">
                                <input
                                    type="file"
                                    id="pictures"
                                    multiple
                                    className="block w-full text-xs lg:text-sm"
                                    accept="image/*"
                                    onChange={handlePictureChange}
                                />
                                <div className="mt-2 grid grid-cols-2 md:grid-cols-4 gap-2">
                                    {/* Render existing pictures */}
                                    {taskData.pictures && taskData.pictures.map((picture, index) => (
                                        <div key={`existing-${index}`} className="relative">
                                            <Image
                                                src={`http://103.159.85.246:4000/${picture}`}
                                                alt={`Existing Picture ${index + 1}`}
                                                width={200}
                                                height={200}
                                                className="w-full h-full cursor-pointer"
                                                onClick={() => handlePicturePreview(picture)}
                                            />
                                            <button
                                                type="button"
                                    ...
                                    {/* Render new pictures */}
                                    {pictureFiles && pictureFiles.map((file, index) => (
                                        <div key={`new-${index}`} className="relative">
                                            ...
                                            >
                                                <FontAwesomeIcon icon={faXmark} className='text-xs rounded-full' />
                                            </button>
                                        </div>
                                    ))}
                                </div>
